...
Чтоб не трахаться постоянно с переключением раскладки туда-сюда, интерфейс решила сделать на английском
Также подумала, что puts в методах, отвечающих за какие-либо действия, быть не должно.
Все puts - это отдельные методы, которые выводят что-то на экран.

Игроки:
класс player и два дочерних класса: human и dealer

Колода генерируется в классе deck
Сами карты - в классе card и кладутся в массив класса deck

Игроки уже умеют набирать начальные карты и делать ставки

Прописаны начало игры и её конец.

Процесс игры - пока пустые методы, но меню уже есть

Очки вроде бы считает. Целиком пока в консоли прогу не проверяла.
Проверила в консоли на простом примере штуку с фетчем. Работает.

Развела в пространстве методы-действия и методы-путсы

Игроки умеют пропускать ход
В метод пропуска я написала отрицание двух других вариантов. Не знаю, насколько это правильно.

Сделала закомментированные разделители между логически отделёнными друг от друга частями кода:
На самом верху - начало игры
Ниже - процесс и действия игрока
Потом - действия робота и его путсы
В самом низу - остальные путсы

Так. Все основные моменты прописаны.
Осталось написать финалочку и переход в метод game over.

Финальные условия прописала.
Прописала переход в game over
...
Плюс оно считает за выигрыш сумму больше 21 очка

Больше не засчитывает.

Когда у игроков по три карты, всё ещё предлагает сделать ход
Прерывает игру после выбора "продолжить"

Дилер всегда берёт карту

Вроде разобралась с условиями. Дилер выполняет действия в зависимости от количества очков.
Почему-то оно выдаёт мне просто карты после приветствия.
А потом пишет карты как объекты с кракозябрами.

Где-то потерялось сообщение о том, что сделал дилер.

Tu, you have 16 points
Bilbo Baggins has 20 points
Bilbo Baggins wins!
...
Tu, your cards: [#<Card:0x00007f88a71eafc8 @value="5", @suit="♠">, #<Card:0x00007f88a71eaa00 @value="3", @suit="♦">]
Your score: 8.
Bank: 20

Bilbo Baggins has 2 cards.
➜  black_jack git:(master) ✗